...
static int send_cb(ompi_request_t *req){
    ...
    if (item != NULL) {
        ...
        mca_coll_adapt_ireduce_context_t * send_context = (mca_coll_adapt_ireduce_context_t *) opal_free_list_wait(context->con->context_list);
    ...
    if (context->con->tree->tree_root != context->con->rank && num_sent == context->con->num_segs) {
        ...
        if (context->con->accumbuf_free != NULL) {
            free(context->con->accumbuf_free);
        }
...
int mca_coll_adapt_ireduce_generic(const void *sbuf, void *rbuf, int count, struct ompi_datatype_t *dtype, struct ompi_op_t *op, int root, struct ompi_communicator_t *comm, ompi_request_t ** request, mca_coll_base_module_t *module, ompi_coll_tree_t* tree){
    ...
    size_t typelng;
    int seg_count = count, num_segs, rank, seg_index, recv_count, send_count, i, j, err, min;
    int * next_recv_segs = NULL;    //store for each child the next segments id need to be received
    ...
    opal_free_list_t * inbuf_list; //a free list contain the incoming message
    ...
    opal_free_list_init(context_list,
                        sizeof(mca_coll_adapt_ireduce_context_t),
                        opal_cache_line_size,
                        OBJ_CLASS(mca_coll_adapt_ireduce_context_t),
    ...
    if (tree->tree_nextsize > 0) {
        ...
        next_recv_segs = (int *)malloc(sizeof(int) * tree->tree_nextsize);
    }
    else {
        inbuf_list = NULL;
        next_recv_segs = NULL;
    ...
    *request = temp_request;
    ...
    mca_coll_adapt_constant_ireduce_context_t *con = OBJ_NEW(mca_coll_adapt_constant_ireduce_context_t);
    con->count = count;
    ...
    con->ongoing_send = 0;
    con->accumbuf_free = accumbuf_free;
        ...
        for( j = 0; j < min; j++ ) {
            ...
            for( i = 0; i < tree->tree_nextsize; i++ ) {
                ...
                if (seg_index < num_segs) {
                    ...
                    //get context
                    mca_coll_adapt_ireduce_context_t * context = (mca_coll_adapt_ireduce_context_t *) opal_free_list_wait(context_list);
                    context->buff = NULL;
