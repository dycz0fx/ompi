...
static int recv_cb(ompi_request_t *req){
    ...
    if (new_id < context->con->num_segs) {
        //get inbuf
        //wei mca_coll_adapt_inbuf_t * inbuf = (mca_coll_adapt_inbuf_t *) opal_free_list_wait(context->con->inbuf_list);
        char *inbuf = NULL;
    ...
    if (context->con->accumbuf[context->frag_id] == NULL) {
        keep_inbuf = 1;
        //context->con->accumbuf[context->frag_id] = context->inbuf->buff;
        context->con->accumbuf[context->frag_id] = context->inbuf;
        //op sbuf and accmbuf to accumbuf
        //wei ompi_op_reduce(context->con->op, context->con->sbuf + (ptrdiff_t)context->frag_id * (ptrdiff_t)context->con->segment_increment, context->con->accumbuf[context->frag_id] - context->con->lower_bound, op_count, context->con->datatype);
        ...
        //wei ompi_op_reduce(context->con->op, context->inbuf->buff - context->con->lower_bound, context->con->accumbuf[context->frag_id] - context->con->lower_bound, op_count, context->con->datatype);
...
int mca_coll_adapt_reduce_generic(const void *sbuf, void *rbuf, int count, struct ompi_datatype_t *dtype, struct ompi_op_t *op, int root, struct ompi_communicator_t *comm, mca_coll_base_module_t *module, ompi_coll_tree_t* tree){
    ...
    if (tree->tree_nextsize > 0) {
        ...
        distance = (char *)temp_inbuf->buff - (char *)temp_inbuf; //address of inbuf->buff to address of inbuf
        ...
        for( j = 0; j < min; j++ ) {
            ...
            for( i = 0; i < tree->tree_nextsize; i++ ) {
                ...
                if (seg_index < num_segs) {
                    ...
                    ompi_request_t *recv_req;
                    err = MCA_PML_CALL(irecv(inbuf - lower_bound, recv_count, dtype, tree->tree_next[i], seg_index, comm, &recv_req));
                    if (MPI_SUCCESS != err) {
                        return err;
                    }
