...
static int recv_cb(ompi_request_t *req){
    ...
    if (new_id < context->con->num_segs) {
       ...
       // mca_coll_adapt_inbuf_t * inbuf = (mca_coll_adapt_inbuf_t *) opal_free_list_wait(context->con->inbuf_list);
        char *inbuf = (char*)opal_cuda_malloc_gpu_buffer(real_seg_size_cuda, 0);
        //get new context item from free list
        //printf("wait context_list\n");
        ...
        OBJ_RETAIN(context->con);
        recv_context->inbuf = (mca_coll_adapt_inbuf_t *)inbuf;
        ...
        MCA_PML_CALL(irecv(inbuf, recv_count, recv_context->con->datatype, recv_context->peer, recv_context->frag_id, recv_context->con->comm, &recv_req));
...
//wei    opal_mutex_lock(context->con->mutex_op_list[context->frag_id]);
 ...
 //                  op_count, context->con->datatype);
    opal_cuda_recude_op_sum_double((void*)context->inbuf, 
                                  context->con->accumbuf + (ptrdiff_t)context->frag_id * (ptrdiff_t)context->con->segment_increment, 
                                  op_count, NULL);
//wei    opal_mutex_unlock(context->con->mutex_op_list[context->frag_id]);
...
//wei    opal_mutex_lock (context->con->mutex_num_recv_segs);
...
//wei        opal_mutex_unlock (context->con->mutex_num_recv_segs);
...
int mca_coll_adapt_reduce_pipeline(const void *sbuf, void *rbuf, int count, struct ompi_datatype_t *dtype, struct ompi_op_t *op, int root, struct ompi_communicator_t *comm, mca_coll_base_module_t *module){
    ...
    printf("i am in reduce pipeline\n");
    int r =  mca_coll_adapt_reduce_generic(sbuf, rbuf, count, dtype, op, root, comm, module, tree);
    ompi_coll_base_topo_destroy_tree(&tree);
    return r;
}
...
int mca_coll_adapt_reduce_generic(const void *sbuf, void *rbuf, int count, struct ompi_datatype_t *dtype, struct ompi_op_t *op, int root, struct ompi_communicator_t *comm, mca_coll_base_module_t *module, ompi_coll_tree_t* tree){
    ...
    printf("seg size %d, nb_segs %d\n", real_seg_size_cuda, num_segs);
    ...
    if (tree->tree_nextsize > 0) {
      ...
      //  cuda_inbuf = (char*)opal_cuda_malloc_gpu_buffer(real_seg_size_cuda * num_segs, 0);
    }
    else {
        inbuf_list = NULL;
        next_recv_segs = NULL;
    ...
    if (tree->tree_nextsize > 0) {
       ...
       //           ompi_datatype_copy_content_same_ddt(dtype, count, (char*)accumbuf, (char*)sbuf);
            ...
            accumbuf_free = (char*)opal_cuda_malloc_gpu_buffer((true_extent + (ptrdiff_t)(count - 1) * extent), 0);
        ...
        for( j = 0; j < min; j++ ) {
            ...
            for( i = 0; i < tree->tree_nextsize; i++ ) {
                ...
                if (seg_index < num_segs) {
                    ...
                    //get inbuf
                    //printf("wait inbuf_list\n");
                    //wei mca_coll_adapt_inbuf_t * inbuf = (mca_coll_adapt_inbuf_t *) opal_free_list_wait(inbuf_list);
                    char *inbuf = (char*)opal_cuda_malloc_gpu_buffer(real_seg_size_cuda, 0);
                    ...
                    context->con = con;
                    OBJ_RETAIN(con);
                    context->inbuf = (mca_coll_adapt_inbuf_t*)inbuf;
                    ...
                    ompi_request_t *recv_req;
                    err = MCA_PML_CALL(irecv(inbuf, recv_count, dtype, tree->tree_next[i], seg_index, comm, &recv_req));
                    if (MPI_SUCCESS != err) {
                        return err;
                    }
    ...
    //leaf nodes
        ...
        for(i = 0; i < min; i++) {
//wei            opal_mutex_lock (mutex_recv_list);
            item = get_next_ready_item(recv_list, tree->tree_nextsize);
//wei            opal_mutex_unlock (mutex_recv_list);
            if (item != NULL) {
    ...
    if (accumbuf_free != NULL) {
     //   free(accumbuf_free);
        opal_cuda_free_gpu_buffer(accumbuf_free, 0);
    }
    OBJ_RELEASE(con);
    ...
    if (cuda_inbuf != NULL) {
        opal_cuda_free_gpu_buffer(cuda_inbuf, 0);
    }
    return MPI_SUCCESS;
}
