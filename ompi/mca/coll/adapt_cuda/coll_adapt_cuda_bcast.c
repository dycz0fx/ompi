        ...
        if (tree->topo_flags == 0 && tree->tree_prev_topo_flags == -1) {   /* root */
            ...
            if (tree->tree_next_topo_flags[context->child_id] != 2) {
                ...
                if (context->con->cpu_buff_memcpy_flags[new_id] == CPU_BUFFER_MEMCPY_NOT_DONE) {
                    //    ompi_datatype_copy_content_same_ddt(send_context->con->datatype, send_count, context->con->cpu_buff_list + new_id * context->con->real_seg_size, (char*)send_context->buff);
        //            opal_output(0, "topo 0 root sendcb memcpy\n");
                    opal_cuda_memcpy_sync(context->con->cpu_buff_list + new_id * context->con->real_seg_size, (char*)send_context->buff, send_count*type_size);
                    context->con->cpu_buff_memcpy_flags[new_id] = CPU_BUFFER_MEMCPY_DONE;
                }
                send_buff = context->con->cpu_buff_list + new_id * context->con->real_seg_size;
            } else {
            ...
            if (tree->tree_next_topo_flags[context->child_id] == 2) {
                ...
                if (context->con->cpu_buff_memcpy_flags[new_id] == CPU_BUFFER_MEMCPY_NOT_DONE) {
                    ...
                    opal_cuda_memcpy_sync(send_context->buff, context->con->cpu_buff_list + new_id * context->con->real_seg_size, send_count*type_size);
                    context->con->cpu_buff_memcpy_flags[new_id] = CPU_BUFFER_MEMCPY_DONE;
                }
                send_buff = send_context->buff;
            } else {
...
static int recv_cb_cpu(ompi_request_t *req){
    ...
    if (new_id < context->con->num_segs) {
       ...
       //     opal_output(0, "recvcb change recv_buff\n");
            recv_buff = context->con->cpu_buff_list + new_id * context->con->real_seg_size;
        } else {
        ...
        //        opal_output(0, "topo 1 recv cb memcpy\n");
                ...
                if (context->con->cpu_buff_memcpy_flags[context->frag_id] == CPU_BUFFER_MEMCPY_NOT_DONE) {
                    if (coll_adapt_cuda_bcast_use_sync) {
                        opal_cuda_memcpy_sync(context->buff, context->con->cpu_buff_list + context->frag_id * context->con->real_seg_size, send_count*type_size);
                        context->con->cpu_buff_memcpy_flags[context->frag_id] = CPU_BUFFER_MEMCPY_DONE;
                    } else {
                        mca_common_cuda_memcpy_async(context->buff, context->con->cpu_buff_list + context->frag_id * context->con->real_seg_size, send_count*type_size);
    ...
    //                opal_output(0, "topo 0 recv cb direct send\n");
                    send_buff = context->con->cpu_buff_list + context->frag_id * context->con->real_seg_size;
                }
            } else {
                send_buff = send_context->buff;
            }
    ...
    /* node / socket leader, if they dont have children or children is 1, make sure, copy data back to gpu mem */
    if ((tree->topo_flags == 1 && tree->tree_prev_topo_flags == 0) || (tree->topo_flags == 0 && tree->tree_prev_topo_flags == 0)) { 
        if (context->con->cpu_buff_memcpy_flags[context->frag_id] == CPU_BUFFER_MEMCPY_NOT_DONE) {
            send_count = context->con->seg_count;
            if (context->frag_id == (context->con->num_segs - 1)) {
...
int mca_coll_adapt_cuda_bcast_nccl(void *buff, int count, struct ompi_datatype_t *datatype, int root, struct ompi_communicator_t *comm, mca_coll_base_module_t *module){
   ...
   // coll_adapt_cuda_nccl_comm_destroy(nccl_comm);
    ...
    return mca_coll_adapt_cuda_bcast_nccl_topo(buff, count, datatype, root, comm, module, coll_comm->cached_topochain);
...
int mca_coll_adapt_cuda_bcast_two_trees_binary(void *buff, int count, struct ompi_datatype_t *datatype, int root, struct ompi_communicator_t *comm, mca_coll_base_module_t *module){
    ...
    if (total_num_segs > 1 && size >= 3) {
        ...
        return mca_coll_adapt_cuda_bcast_two_trees_generic(buff, count, datatype, root, comm, module, coll_comm->cached_two_trees_binary);
    }
    else{
        return mca_coll_adapt_cuda_bcast_binary(buff, count, datatype, root, comm, module);
    }
}
...
int mca_coll_adapt_cuda_bcast_two_trees_binomial(void *buff, int count, struct ompi_datatype_t *datatype, int root, struct ompi_communicator_t *comm, mca_coll_base_module_t *module){
    ...
    if (total_num_segs > 1 && size >= 3) {
        ...
        return mca_coll_adapt_cuda_bcast_two_trees_generic(buff, count, datatype, root, comm, module, coll_comm->cached_two_trees_binomial);
    }
    else{
        return mca_coll_adapt_cuda_bcast_binomial(buff, count, datatype, root, comm, module);
    }
}
...
int mca_coll_adapt_cuda_bcast_two_chains(void *buff, int count, struct ompi_datatype_t *datatype, int root, struct ompi_communicator_t *comm, mca_coll_base_module_t *module){
    ...
    if (total_num_segs > 1 && size >= 3) {
        ...
        return mca_coll_adapt_cuda_bcast_two_trees_generic(buff, count, datatype, root, comm, module, coll_comm->cached_two_chains);
    }
    else{
        return mca_coll_adapt_cuda_bcast_binomial(buff, count, datatype, root, comm, module);
    }
...
int mca_coll_adapt_cuda_bcast_nccl_topo(void *buff, int count, struct ompi_datatype_t *datatype, int root, struct ompi_communicator_t *comm, mca_coll_base_module_t *module, ompi_coll_tree_t* tree){
    ...
    if (rank == root){
        ...
        if (tree->tree_nextsize == 1) {
            ...
            for (i = 0; i < num_segs; i++) {
                ...
                coll_adapt_cuda_nccl_bcast((char *)buff + i * real_seg_size, send_count, ncclChar, root, nccl_comm, stream);
            }
        } else {
            ...
            ompi_request_t *send_req;
            send_count = seg_count;             //number of datatype in each send
            for (i = 0; i < min; i++) {
                if (i == (num_segs - 1)) {
                    send_count = count - i * seg_count;
                }
                for (j=0; j<tree->tree_nextsize; j++) {
                  ...
                  //  context->buff_tmp = NULL;
                    ...
                    // send to next node leader
                    if (tree->topo_flags != -1 && tree->tree_next_topo_flags[j] == 0) {
                        ...
                        coll_adapt_cuda_nccl_bcast(send_buff, send_count, ncclChar, root, nccl_comm, stream);
                    }
    ...
    //if not root, receive data from parent in the tree.
        ...
        // SM
        if (tree->tree_nextsize == 1 || tree->tree_nextsize == 0) {
            send_count = seg_count;
            for (i = 0; i < num_segs; i++) {
                if (i == (num_segs - 1)) {
                    send_count = count - i * seg_count;
            ...
            for (i = 0; i < min; i++) {
             ...
             //   context->buff_tmp = NULL;
                ...
                if (tree->topo_flags == 0 && tree->tree_prev_topo_flags == 0) {
                    ...
                    if (rank < 4) {
                        coll_adapt_cuda_nccl_bcast(recv_buff, recv_count, ncclChar, root, nccl_comm, stream);
                    }
                }
            }
            opal_mutex_unlock(mutex);
    ...
    if (con->cpu_buff_list != NULL) {
        mpool->mpool_free(mpool, con->cpu_buff_list);
        con->cpu_buff_list = NULL;
    }
        ...
        for (i = 0; i < min; i++) {
            ...
            for (j=0; j<tree->tree_nextsize; j++) {
                ...
                if (tree->topo_flags != -1 && tree->tree_next_topo_flags[j] != 2) {
                    if (con->cpu_buff_list == NULL) {
                        con->cpu_buff_list = (char*)malloc(sizeof(void*) * num_segs);
                        con->cpu_buff_list = mpool->mpool_alloc(mpool, sizeof(char)* real_seg_size * num_segs, 0, 0);
                    ...
                    if (con->cpu_buff_memcpy_flags[i] == CPU_BUFFER_MEMCPY_NOT_DONE) {
                       // ompi_datatype_copy_content_same_ddt(datatype, send_count, (char*)cpu_buff_list + i * real_seg_size, (char*)context->buff);
                        opal_cuda_memcpy_sync(con->cpu_buff_list + i * real_seg_size, (char*)context->buff, send_count*type_size);
                        con->cpu_buff_memcpy_flags[i] = CPU_BUFFER_MEMCPY_DONE;
                    }
                    send_buff = con->cpu_buff_list + i * real_seg_size;
                } else {
                    send_buff = context->buff;
                }
...
{
    mca_coll_adapt_cuda_bcast_two_trees_context_t *context = (mca_coll_adapt_cuda_bcast_two_trees_context_t *) req->req_complete_cb_data;
    ...
    if (sent_id < context->con->num_recv_segs[context->tree]) {
        ...
        mca_coll_adapt_cuda_bcast_two_trees_context_t * send_context = (mca_coll_adapt_cuda_bcast_two_trees_context_t *) opal_free_list_wait(context->con->context_lists[context->tree]);
        send_context->buff = context->buff + (new_id - context->frag_id) * context->con->real_seg_size;
        send_context->frag_id = new_id;
        send_context->child_id = context->child_id;
        send_context->peer = context->peer;
...
static int two_trees_recv_cb(ompi_request_t *req){
    ...
    if (new_position < context->con->num_segs[context->tree]) {
        ...
        //get new context item from free list
        mca_coll_adapt_cuda_bcast_two_trees_context_t * recv_context = (mca_coll_adapt_cuda_bcast_two_trees_context_t *) opal_free_list_wait(context->con->context_lists[context->tree]);
        recv_context->buff = context->buff + (new_id - context->frag_id) * context->con->real_seg_size;
        recv_context->frag_id = new_id;
        recv_context->child_id = context->child_id;
        recv_context->peer = context->peer;
...
int mca_coll_adapt_cuda_bcast_two_trees_generic(void *buff, int count, struct ompi_datatype_t *datatype, int root, struct ompi_communicator_t *comm, mca_coll_base_module_t *module, ompi_coll_tree_t** trees){
    ...
    opal_free_list_init(context_lists[0],
                        sizeof(mca_coll_adapt_cuda_bcast_two_trees_context_t),
                        opal_cache_line_size,
                        OBJ_CLASS(mca_coll_adapt_cuda_bcast_two_trees_context_t),
                        0,opal_cache_line_size,
                        FREE_LIST_NUM,
                        FREE_LIST_MAX,
                        FREE_LIST_INC,
    ...
    //Set constant context for send and recv call back
    mca_coll_adapt_cuda_constant_bcast_two_trees_context_t *con = OBJ_NEW(mca_coll_adapt_cuda_constant_bcast_two_trees_context_t);
    con->count = count;
    con->seg_count = seg_count;
    con->datatype = datatype;
    con->comm = comm;
        ...
        for (t = 0; t < 2; t++) {
            ...
            for (i = 0; i < min[t]; i++) {
                ...
                for (j=0; j<trees[t]->tree_nextsize; j++) {
                    mca_coll_adapt_cuda_bcast_two_trees_context_t * context = (mca_coll_adapt_cuda_bcast_two_trees_context_t *) opal_free_list_wait(context_lists[t]);
