 ...
 * Global variables
 */
uint32_t mca_coll_adapt_cuda_one = 1;


/*
 * Local functions
 */
static int adapt_cuda_module_enable(mca_coll_base_module_t *module,
                                    struct ompi_communicator_t *comm);
static int bootstrap_comm(ompi_communicator_t *comm,
                          mca_coll_adapt_cuda_module_t *module);

/*
 * Module constructor
 */
static void mca_coll_adapt_cuda_module_construct(mca_coll_adapt_cuda_module_t *module)
{
 ...
 * Module destructor
 */
static void mca_coll_adapt_cuda_module_destruct(mca_coll_adapt_cuda_module_t *module)
{
    mca_coll_adapt_cuda_comm_t *c = module->adapt_comm_data;
...
OBJ_CLASS_INSTANCE(mca_coll_adapt_cuda_module_t,
                   mca_coll_base_module_t,
                   mca_coll_adapt_cuda_module_construct,
                   mca_coll_adapt_cuda_module_destruct);
 ...
 * once.
 */
int mca_coll_adapt_cuda_init_query(bool enable_progress_threads,
 ...
 */
mca_coll_base_module_t *
mca_coll_adapt_cuda_comm_query(struct ompi_communicator_t *comm, int *priority)
{
    mca_coll_adapt_cuda_module_t *adapt_module;
    ...
    if (OMPI_COMM_IS_INTER(comm) || 1 == ompi_comm_size(comm)) {
        opal_output_verbose(10, ompi_coll_base_framework.framework_output,
                            "coll:adapt:cuda:comm_query (%d/%s): intercomm, comm is too small, disqualifying myself", comm->c_contextid, comm->c_name);
	return NULL;
    }

    /* Get the priority level attached to this module. If priority is less
     * than or equal to 0, then the module is unavailable. */
    *priority = mca_coll_adapt_cuda_component.adapt_cuda_priority;
    if (mca_coll_adapt_cuda_component.adapt_cuda_priority <= 0) {
        opal_output_verbose(10, ompi_coll_base_framework.framework_output,
                            "coll:adapt:cuda:comm_query (%d/%s): priority too low; disqualifying myself", comm->c_contextid, comm->c_name);
	return NULL;
    }

    adapt_module = OBJ_NEW(mca_coll_adapt_cuda_module_t);
    ...
    adapt_module->super.coll_module_enable = adapt_cuda_module_enable;
    ...
    adapt_module->super.coll_bcast      = mca_coll_adapt_cuda_bcast;
    adapt_module->super.coll_exscan     = NULL;
    adapt_module->super.coll_gather     = NULL;
    adapt_module->super.coll_gatherv    = NULL;
    adapt_module->super.coll_reduce     = mca_coll_adapt_cuda_reduce;
...
static int adapt_cuda_module_enable(mca_coll_base_module_t *module,
                                    struct ompi_communicator_t *comm)
{
    /* We do everything lazily in ompi_coll_adapt_cuda_enable() */
    return OMPI_SUCCESS;
}

int ompi_coll_adapt_cuda_lazy_enable(mca_coll_base_module_t *module,
                                     struct ompi_communicator_t *comm)
{
    int ret;
    int rank = ompi_comm_rank(comm);
    int size = ompi_comm_size(comm);
    mca_coll_adapt_cuda_module_t *adapt_module = (mca_coll_adapt_cuda_module_t*) module;
    size_t control_size, frag_size;
    mca_coll_adapt_cuda_component_t *c = &mca_coll_adapt_cuda_component;
#if OPAL_HAVE_HWLOC
    ...
    if (NULL == maffinity) {
        opal_output_verbose(10, ompi_coll_base_framework.framework_output,
                            "coll:adapt:cuda:lazy_enable (%d/%s): malloc failed (1)",
                            comm->c_contextid, comm->c_name);
        return OMPI_ERR_OUT_OF_RESOURCE;
    }
#endif
...
int mca_coll_adapt_cuda_ft_event(int state) {
