...
static int send_cb(ompi_request_t *req){
    ...
    if (item != NULL) {
        ...
        if (0 == coll_adapt_cuda_reduce_use_sync && context->con->tree->tree_nextsize > 0) {
            ...
            if (0 == mca_common_cuda_query_op_event_item(item->op_event)) {
             //   opal_output(0, "event %p is not done, record it\n", item->op_event);
                send_context->flags = COLL_ADAPT_CUDA_CONTEXT_FLAGS_REDUCE;
            ...
            if (coll_adapt_cuda_use_cpu_buff && (send_context->con->tree->topo_flags == 1 || send_context->con->tree->topo_flags == 0)) {
                ...
                if (send_context->con->tree->tree_nextsize == 0) {
                    ...
                    temp_send_buf = send_context->con->accumbuf_cpu[send_context->frag_id];
                }
                assert(temp_send_buf != NULL);
            }
            TEST("[%d]: In send_cb, create isend to seg %d, peer %d\n", send_context->con->rank, send_context->frag_id, send_context->peer);
    ...
    if (context->con->tree->tree_root != context->con->rank && num_sent == context->con->num_segs) {
        ...
        if (context->con->accumbuf_cpu != NULL) {
            if (context->con->rank != context->con->root) {
                for (i=0; i<context->con->num_segs; i++) {
                    if (context->con->accumbuf_cpu[i]!= NULL) {
                        mpool->mpool_free(mpool, context->con->accumbuf_cpu[i]);
                        context->con->accumbuf_cpu[i] = NULL;
                    }
                //    opal_free_list_return(context->con->inbuf_list, (opal_free_list_item_t*)to_inbuf(context->con->accumbuf[i], context->con->distance));
                }
            }
            free(context->con->accumbuf_cpu);
        }
        ...
        if (context->con->cpu_buff_memcpy_event != NULL) free(context->con->cpu_buff_memcpy_event);
...
static int recv_cb(ompi_request_t *req){
    ...
    if (new_id < context->con->num_segs) {
        ...
        if (coll_adapt_cuda_use_cpu_buff && recv_context->con->tree->topo_flags == 0) {
            if (recv_context->con->rank != recv_context->con->root && recv_context->con->accumbuf_cpu[recv_context->frag_id] == NULL) {
                recv_context->con->accumbuf_cpu[recv_context->frag_id] = mpool->mpool_alloc(mpool, sizeof(char)* recv_context->con->real_seg_size, 0, 0);
                recv_context->con->datatype->super.flags |= OPAL_DATATYPE_FLAG_GPU_ASYNC;
                ompi_datatype_copy_content_same_ddt(recv_context->con->datatype, recv_count, recv_context->con->accumbuf_cpu[recv_context->frag_id], 
                                                    recv_context->con->sbuf + (ptrdiff_t)recv_context->frag_id * (ptrdiff_t)recv_context->con->segment_increment);
                recv_context->con->datatype->super.flags &= ~OPAL_DATATYPE_FLAG_GPU_ASYNC;
    ...
    /* root copy back to gpu to do op */
    if (coll_adapt_cuda_use_cpu_buff && context->con->rank == context->con->root && context->con->tree->tree_next_topo_flags[context->child_id] != 2) {
    ...
    /* node and socket leader, do op in cpu */
    if (coll_adapt_cuda_use_cpu_buff && (context->con->tree->topo_flags == 1 || context->con->tree->topo_flags == 0) && (context->con->rank != context->con->root) ) {
        /* from his own group, copy data into cpu buf */
        if (context->con->tree->tree_next_topo_flags[context->child_id] == 2) {
            if (context->con->cpu_buff_list[context->frag_id * context->con->tree->tree_nextsize + context->child_id] == NULL) {
                context->con->cpu_buff_list[context->frag_id * context->con->tree->tree_nextsize + context->child_id] = mpool->mpool_alloc(mpool, sizeof(char)* context->con->real_seg_size, 0, 0);
            }
            ompi_datatype_copy_content_same_ddt(context->con->datatype, context->count, context->con->cpu_buff_list[context->frag_id * context->con->tree->tree_nextsize + context->child_id], (char*)context->buff);
        }
        if (context->con->cpu_buff_memcpy_event[context->frag_id] != NULL) {
            mca_common_cuda_sync_op_event_item(context->con->cpu_buff_memcpy_event[context->frag_id]);
            mca_common_cuda_return_op_event_item(context->con->cpu_buff_memcpy_event[context->frag_id]);
            context->con->cpu_buff_memcpy_event[context->frag_id] = NULL;
        }
        assert(context->con->cpu_buff_list[context->frag_id * context->con->tree->tree_nextsize + context->child_id] != NULL);
        assert(context->con->accumbuf_cpu[context->frag_id] != NULL);
        ...
        if (context->con->accumbuf[context->frag_id] == NULL) {
           ...
           // ompi_op_reduce(context->con->op, context->con->sbuf + (ptrdiff_t)context->frag_id * (ptrdiff_t)context->con->segment_increment, context->con->accumbuf[context->frag_id], op_count, context->con->datatype);
            ...
            if (context->inbuf == NULL) {
                ...
                //free old accumbuf
                if (0 == coll_adapt_cuda_reduce_use_sync && context->con->rank != context->con->root) {
                    buff_to_free = context->con->accumbuf[context->frag_id];
                } else {
                ...
                opal_cuda_recude_op_sum_double(context->inbuf - context->con->lower_bound, context->con->accumbuf[context->frag_id], op_count, op_cuda_stream);
            }
        }
    ...
    if (context->con->tree->tree_root == context->con->rank && num_recv_segs_t == context->con->num_segs * context->con->tree->tree_nextsize) {
        ...
        opal_mutex_unlock (context->con->mutex_num_recv_segs);
        if (!keep_inbuf && context->inbuf != NULL && !op_asyn_not_free) {
            TEST("return inbuf\n");
            opal_cuda_free_gpu_buffer(context->inbuf, 0);
            context->inbuf = NULL;
           // opal_free_list_return(context->con->inbuf_list, (opal_free_list_item_t*)context->inbuf);
...
int mca_coll_adapt_cuda_reduce_generic(const void *sbuf, void *rbuf, int count, struct ompi_datatype_t *dtype, struct ompi_op_t *op, int root, struct ompi_communicator_t *comm, mca_coll_base_module_t *module, ompi_coll_tree_t* tree){
    ...
    con->accumbuf_cpu = NULL;
    con->cpu_buff_memcpy_event = NULL;
                ...
                if (seg_index < num_segs) {
                    ...
                    if (coll_adapt_cuda_use_cpu_buff && (tree->topo_flags == 1 || tree->topo_flags == 0)) {
                        if (con->cpu_buff_list == NULL) {
                            ...
                            con->cpu_buff_memcpy_event = (void **)malloc(sizeof(void*) * num_segs);
                            for (k = 0; k < num_segs * tree->tree_nextsize; k++) {
                                con->cpu_buff_list[k] = NULL;
                            }
                            for (k = 0; k < num_segs; k++) {
                                con->cpu_buff_memcpy_event[k] = NULL;
                            }
                        }
                    }
                    /* node leader, receive to cpu buffer from other node and socket*/
                    if (coll_adapt_cuda_use_cpu_buff && tree->topo_flags == 0) {
                        if (rank != root && con->accumbuf_cpu[context->frag_id] == NULL) {
                            con->accumbuf_cpu[context->frag_id] = mpool->mpool_alloc(mpool, sizeof(char)* real_seg_size, 0, 0);
...
static int reduce_async_op_free_inbuf(mca_coll_adapt_cuda_item_t * item, int rank)
{
    ...
    for (i = 0; i < item->count; i++) {
        if (item->buff_to_free[i] != NULL) {
            opal_cuda_free_gpu_buffer(item->buff_to_free[i], 0);
