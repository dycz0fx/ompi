...
mca_coll_han_allgather_intra(const void *sbuf, int scount,
    ...
    ompi_communicator_t *low_comm = han_module->cached_low_comms[mca_coll_han_component.han_allgather_low_module];
    ompi_communicator_t *up_comm = han_module->cached_up_comms[mca_coll_han_component.han_allgather_up_module];
    ...
    mac_coll_han_set_allgather_argu(lg_argu, lg, (char *)sbuf, NULL, scount, sdtype, rbuf, rcount, rdtype, root_low_rank, up_comm, low_comm, w_rank, low_rank!=root_low_rank, han_module->is_mapbycore, topo, temp_request);
    /* init lg task */
...
int mca_coll_han_allgather_uag_task(void *task_argu){
    ...
    if (t->noop) {
        ...
        if (t->is_mapbycore) {
            OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output, "[%d]: HAN Allgather is bycore: ", t->w_rank));
        ...
        OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output, "[%d] HAN Allgather:  ug allgather finish\n", t->w_rank));
        ...
        if (!t->is_mapbycore) {
            ...
            for (i=0; i<up_size; i++) {
                for (j=0; j<low_size; j++) {
                    OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output, "[%d]: HAN Allgather copy from %d %d\n", t->w_rank, (i*low_size+j)*2+1, t->topo[(i*low_size+j)*2+1]));
