...
mca_coll_han_allreduce_intra(const void *sbuf,
    ...
    if (mca_coll_han_component.han_auto_tune && mca_coll_han_component.han_auto_tuned != NULL) {
        ...
        uint32_t id = n*mca_coll_han_component.han_auto_tune_c*mca_coll_han_component.han_auto_tune_m + c*mca_coll_han_component.han_auto_tune_m + m + mca_coll_han_component.han_auto_tune_n*mca_coll_han_component.han_auto_tune_c*mca_coll_han_component.han_auto_tune_m;
        ...
        COLL_BASE_COMPUTED_SEGCOUNT(mca_coll_han_component.han_allreduce_segsize, typelng, seg_count);
    }
    
    /* Determine number of elements sent per task. */
    OPAL_OUTPUT_VERBOSE((10, mca_coll_han_component.han_output, "In HAN Allreduce seg_size %d seg_count %d count %d\n", mca_coll_han_component.han_allreduce_segsize, seg_count, count));
    ...
    mac_coll_han_set_allreduce_argu(t, t0, (char *)sbuf, (char *)rbuf, seg_count, dtype, op, root_up_rank, root_low_rank, up_comm, low_comm, num_segments, 0, w_rank, count-(num_segments-1)*seg_count, low_rank!=root_low_rank, NULL, completed);
    /* init t0 task */
...
int mca_coll_han_allreduce_t0_task(void *task_argu){
    ...
    OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output, "[%d] HAN Allreduce:  t0 %d r_buf %d\n", t->w_rank, t->cur_seg, ((int *)t->rbuf)[0]));
...
int mca_coll_han_allreduce_t1_task(void *task_argu){
    mca_allreduce_argu_t *t = (mca_allreduce_argu_t *)task_argu;
    OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output, "[%d] HAN Allreduce:  t1 %d r_buf %d\n", t->w_rank, t->cur_seg, ((int *)t->rbuf)[0]));
    OBJ_RELEASE(t->cur_task);
    ptrdiff_t extent, lb;
    ompi_datatype_get_extent(t->dtype, &lb, &extent);
...
int mca_coll_han_allreduce_t2_task(void *task_argu){
    mca_allreduce_argu_t *t = (mca_allreduce_argu_t *)task_argu;
    OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output, "[%d] HAN Allreduce:  t2 %d r_buf %d\n", t->w_rank, t->cur_seg, ((int *)t->rbuf)[0]));
    OBJ_RELEASE(t->cur_task);
    ptrdiff_t extent, lb;
    ompi_datatype_get_extent(t->dtype, &lb, &extent);
...
int mca_coll_han_allreduce_t3_task(void *task_argu){
    mca_allreduce_argu_t *t = (mca_allreduce_argu_t *)task_argu;
    OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output, "[%d] HAN Allreduce:  t3 %d r_buf %d\n", t->w_rank, t->cur_seg, ((int *)t->rbuf)[0]));
    OBJ_RELEASE(t->cur_task);
    ptrdiff_t extent, lb;
    ompi_datatype_get_extent(t->dtype, &lb, &extent);
    ompi_request_t *reqs[2];
    ...
    t->completed[0]++;
    OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output, "[%d] HAN Allreduce:  t3 %d total %d\n", t->w_rank, t->cur_seg, t->completed[0]));
