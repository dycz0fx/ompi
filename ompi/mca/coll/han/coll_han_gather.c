                              ...
                              mca_coll_base_module_t *module)
{
    ...
    ompi_communicator_t *low_comm = han_module->cached_low_comms[mca_coll_han_component.han_gather_low_module];
    ompi_communicator_t *up_comm = han_module->cached_up_comms[mca_coll_han_component.han_gather_up_module];
    ...
    OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output, "[%d]: Future Gather root %d root_low_rank %d root_up_rank %d\n", w_rank, root, root_low_rank, root_up_rank));
    ...
    if (w_rank == root) {
        ...
        if (han_module->is_mapbycore) {
            OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output, "[%d]: Future Gather is_bycore: ", w_rank));
    ...
    mac_coll_han_set_gather_argu(lg_argu, lg, (char *)sbuf, NULL, scount, sdtype, reorder_rbuf, rcount, rdtype, root, root_up_rank, root_low_rank, up_comm, low_comm, w_rank, low_rank!=root_low_rank, temp_request);
    /* init lg task */
    ...
    if (w_rank == root && !han_module->is_mapbycore) {
        for (i=0; i<up_size; i++) {
            for (j=0; j<low_size; j++) {
                OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output, "[%d]: Future Gather copy from %d %d\n", w_rank, (i*low_size+j)*2+1, topo[(i*low_size+j)*2+1]));
...
int mca_coll_han_gather_ug_task(void *task_argu){
    ...
    if (t->noop) {
        OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output, "[%d] Future Gather:  ug noop\n", t->w_rank));
    }
    else {
        int low_size = ompi_comm_size(t->low_comm);
        /* inter node gather */
