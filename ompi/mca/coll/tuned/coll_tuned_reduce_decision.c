...
int ompi_coll_tuned_reduce_intra_check_forced_init (coll_tuned_force_algorithm_mca_param_indices_t *mca_param_indices)
{
    ...
    mca_param_indices->algorithm_param_index =
        mca_base_component_var_register(&mca_coll_tuned_component.super.collm_version,
                                        "reduce_algorithm",
                                        "Which reduce algorithm is used. Can be locked down to choice of: 0 ignore, 1 linear, 2 chain, 3 pipeline, 4 binary, 5 binomial, 6 in-order binary, 7 topology-aware chain",
                                         ...
                                         int segsize, int max_requests )
{
    ...
    case (6):  return ompi_coll_base_reduce_intra_in_order_binary(sbuf, rbuf, count, dtype,
                                                                  op, root, comm, module,
                                                                  segsize, max_requests);
    case (7): return ompi_coll_base_reduce_intra_topoaware_chain(sbuf, rbuf, count, dtype,
                                                                     op, root, comm, module,
                                                                     segsize, max_requests);
    } /* switch */
